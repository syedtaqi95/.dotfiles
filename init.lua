...
require("lazy").setup({
	...
	-- Fuzzy Finder (files, lsp, etc)
	{
		...
		-- NOTE: `0.1.x` branch is incompatible with nvim-treesitter `main` branch
		-- (`ft_to_lang` nil error in previewer). Track `master` which uses the
		-- built-in `vim.treesitter.language.get_lang()` API.
		dependencies = {
...

	{
		...
		config = function()
			...
			-- Enable treesitter highlighting + indent per-buffer.
			-- The `main` branch no longer provides `highlight = { enable = true }`
			-- or `ensure_installed` via `nvim-treesitter.configs.setup`.
			vim.api.nvim_create_autocmd("FileType", {
				group = vim.api.nvim_create_augroup("treesitter-enable", { clear = true }),
				callback = function(args)
					local buf = args.buf
					-- Enable highlighting (don't error on missing parsers)
			...
			-- MDX: treat as markdown for highlighting
			vim.filetype.add({
				extension = {
					mdx = "mdx",
				},
...
vim.keymap.set("n", "<leader>gl", function()
	vim.cmd(
		"Flog -limit=" .. vim.fn.line(".") .. "," .. vim.fn.line(".") .. ":" .. vim.fn.fnameescape(vim.fn.expand("%"))
	)
...
-- NOTE: nvim-treesitter `main` branch handles parser installation in its
-- plugin spec above. Highlighting/indent are enabled per-buffer via a
-- FileType autocmd there (no `nvim-treesitter.configs.setup` anymore).
